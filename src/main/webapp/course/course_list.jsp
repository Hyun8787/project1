...
<br><br><br>
	...
	<div class="container searchtal">
		...
		<span>전체 강의수 : <%=listCount%> 개</span>
		<a href="courseWriteForm.me"><button type="button" class="btn btn-outline-primary">강의 등록</button></a>
		</div>
			<%
...
<!-- <hr id="trline"> -->
    ...
    <div id="title" class="c_title"><%if(co.getC_NO() >0) %>
    ...
    <div class="c_resaldate"><%if(co.getC_RESALTDATE() != null) {%> <%=co.getC_RESALTDATE() %>
				<%}else{%>&nbsp;<%} %></div>
</div>
<hr>
			<%} %>
...
<hr id="trline">
    ...
    <div class="c_no">번호 : <%=co.getC_NO()%></div>
    <div class="c_title">강의명 : <%if(co.getC_NO() >0) %>
					<a href="courseDetail.me?c_no=<%=co.getC_NO()%>&page=<%=nowPage%>">
						<%=co.getC_TITLE()%></a></div>
    <div class="p_name">교수명 : <%=co.getP_NAME()%></div>
    <div class="c_date">개강일자 : <%=co.getC_DATE()%></div>
    <div class="c_resdate">등록일 : <%=co.getC_RESDATE()%></div>
    <div class="c_resaldate">최종수정일 : <%if(co.getC_RESALTDATE() != null) {%> <%=co.getC_RESALTDATE() %>
				<%}else{%>&nbsp;<%} %></div>
</div>
<hr>
			<%} %>
...
<!-- 	페이징 처리 -->
	<nav aria-label="Page navigation example" id="pageList">
	  <ul class="pagination justify-content-center">
		...
		<%}else{ %>
		 <li class="page-item">
		 <a class="page-link" href="courseList.me?page=<%=nowPage-1 %>" aria-label="Previous">
		...
		<%for(int a=startPage;a<=endPage;a++){
 				if(a==nowPage){%> 
 				 <li class="page-item active" aria-current="page"><a class="page-link" href="courseList.me?page=<%=a%>"><%=a %></a></li>
		<%}else{ %>
		<li class="page-item"><a class="page-link" href="courseList.me?page=<%=a%>"><%=a %></a></li>
		<%} %>
		<%} %>
		...
		<%}else{ %>
		<li class="page-item">
		 <a href="courseList.me?page=<%=nowPage+1 %>" class="page-link" aria-label="Next">
