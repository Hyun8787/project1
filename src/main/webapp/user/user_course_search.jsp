...
<div class="container searchtal">
				...
				if("".equals(request.getParameter("searchText"))){%>
					<span>전체 검색결과  |  총 <%=listCount%>강</span>
				<%
				}else{
				%>
				<span>"<%=request.getParameter("searchText")%>" 검색결과 | 총 <%=listCount%>강</span>	
				<% }%>
				</div>
...
<!-- 						개강일자 -->
						<div class="badge bg-dark text-white position-absolute" style="top:0.5rem; left: 0.5rem">개강일자 <%=co.getC_DATE()%>
						</div>
<!-- 						강좌수 -->
						<div class="badge bg-dark text-white position-absolute" style="bottom: 14.2rem; right: 0.5rem"><%=co.getC_VOL()%>강
						</div>
						<!-- 강의 이미지 -->
	...
	<nav aria-label="Page navigation example" id="pageList">
		<ul class="pagination justify-content-center">
			...
			<li class="page-item"><a class="page-link"
				href="usersearchAction.me?C_COL=<%=request.getParameter("C_COL") %>&searchText=<%=request.getParameter("searchText")%>&pageNumber=<%=nowPage-1 %>" aria-label="Previous">
			...
			<li class="page-item active" aria-current="page"><a
				class="page-link" 
				href="usersearchAction.me?C_COL=<%=request.getParameter("C_COL") %>&searchText=<%=request.getParameter("searchText")%>&pageNumber=<%=a%>"><%=a%>
				</a></li>
			<%
			} else {
			%>
			...
			%>
			<li class="page-item">
			<a href="usersearchAction.me?C_COL=<%=request.getParameter("C_COL") %>&searchText=<%=request.getParameter("searchText")%>&pageNumber=<%=nowPage+1 %>" class="page-link" aria-label="Next">
